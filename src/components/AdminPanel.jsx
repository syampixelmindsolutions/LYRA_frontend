...
const AdminPanel = () => {
  ...
  // GET /api/products  — expects: array of product objects
  ...
  // GET /api/customers  — expects: array of customer objects
  ...
  // GET /api/analytics  — expects: { revenue: [{month, revenue, orders}], categories: [{cat, pct, revenue}], metrics: {...} }
  ...
  // GET /api/settings  — expects: { storeName, supportEmail, supportPhone, gstNumber, freeDeliveryThreshold, standardDeliveryFee, expressDeliveryFee, estimatedDeliveryDays, notifications: {...} }
  const fetchSettings = useCallback(async () => {
    setLoad("settings", true); setError("settings", null);
    try {
      const data = await apiFetch("/settings");
      setSettings(data);
    } catch (e) {
      setError("settings", e.message);
    } finally {
  ...
  // ─────────────────────────────────────────────────────────────────
  // SETTINGS — save
  // PUT /api/settings   body: settings object
  // ─────────────────────────────────────────────────────────────────
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const updated = await apiFetch("/settings", { method: "PUT", body: JSON.stringify(settingsForm) });
      setSettings(updated);
                      ...
                      {filteredOrders.map((order) => {
                        ...
                        return (
                          <tr key={id} className="hover:bg-white/3 transition-colors">
                            ...
                            <td className="px-4 py-3.5 text-white/60 text-xs">{order.items || order.itemCount || "—"}</td>
