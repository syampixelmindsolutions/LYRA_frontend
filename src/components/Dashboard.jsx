...
//   // Filter products based on active tab, search and brand
//   const filtered = products.filter((p) => {
//     const matchTab = activeTab === "All" || p.category === activeTab;
//     const matchSearch = p.name.toLowerCase().includes(search.toLowerCase());
//     const matchBrand = !activeBrand || p.brand?.toLowerCase() === activeBrand.toLowerCase();
...
//     return (
//       <div className={`relative h-72 bg-gradient-to-r ${banner.colorFrom || 'from-purple-900'} ${banner.colorVia || 'via-violet-800'} ${banner.colorTo || 'to-purple-600'} shadow-xl flex`}>
//         <div className="relative z-10 flex flex-col justify-center px-12 w-1/2 flex-shrink-0">
//           <p className="text-xs tracking-widest text-white/60 uppercase mb-2">✦ Lyra Exclusive</p>
//           <h1 className="font-serif text-4xl font-light text-white leading-tight mb-2">{banner.title}</h1>
//           <p className="text-white/70 text-sm mb-6">{banner.subtitle}</p>
...
//       {/* Navbar - now sticky below the banner */}
//         <nav className={`sticky top-0 z-50 bg-white shadow-md transition-all ${scrolled ? 'shadow-lg' : ''}`}>
//           <div className="max-w-screen-xl mx-auto px-6 h-16 flex items-center gap-6">
//             {/* Logo */}
//             <div
//               className="font-serif text-2xl tracking-widest text-[#1e0a3c] cursor-pointer flex-shrink-0 select-none"
//               onClick={() => { navigate("/dashboard"); setActiveBrand(null); }}
//             >
//               LY<span className="text-pink-400">R</span>A
//             </div>
...
//             <div className="flex-1 max-w-md relative">
//               <input
//                 className="w-full bg-gray-100 text-gray-900 placeholder-gray-400 rounded-xl px-4 py-2 pr-10 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-violet-400 transition-all duration-200 border border-transparent"
//                 placeholder="Search clothes, shoes..."
//                 value={search}
//                 onChange={(e) => { setSearch(e.target.value); setActiveBrand(null); }}
//               />
...
//               >
//                 <span className="text-xl">{wishlistCount > 0 ? "♥" : "♡"}</span>
//                 {wishlistCount > 0 && (
...
//                 >
//                   <img
//                     src={user.avatar || "https://placehold.co/40x40/f3f4f6/9ca3af?text=👤"}
//                     alt="Profile"
...
//                       <p className="text-gray-900 font-semibold text-sm">{user.fullName || "Guest"}</p>
...
//         {/* Banner section */}
//         <div className={`transition-all duration-300 ${scrolled ? 'h-0 overflow-hidden' : 'h-72'}`}>
//           {loadingBanners ? (
//             <div className="relative h-72 bg-gradient-to-r from-purple-100 to-purple-200 animate-pulse flex items-center justify-center">
//               <p className="text-purple-400">Loading banners...</p>
//             </div>
//           ) : banners.length > 0 ? (
//             <HeroBanner banner={banners[currentBanner]} />
//           ) : (
//             <div className="relative h-72 bg-gradient-to-r from-purple-900 to-pink-600 flex items-center justify-center">
//               <p className="text-white">No banners available</p>
//             </div>
//           )}
//         </div>
